...
# Arguments
- `templates::Templates`: the templates that model the attacked vectors
...
"""
function single_byte_template_attack(templates::Templates, attack_vectors::AbstractArray; fun = single_load_instruction, sample_function = sample_function, N = 2^10)
    keyGuesses = []
    for value = 0:255
        profiled_vectors = zeros(Float64, (length(sample_function(templates, fun, value)), N))
        #display(profiled_vectors)

        for i = 1:N
            profiled_vectors[:,i] = sample_function(templates, fun, value)
        end
        #display(profiled_vectors)
...
# Arguments
- `templates::Templates`: the template that models the attack attacked vectors
...
"""
function multi_byte_template_attack(templates::Templates, attack_vectors, byte_amount::Integer; fun = multi_load_instructions, N = 2^10)
    key = []
    for byte = 1:byte_amount
        attack_function = n_wrap_sampler(byte, byte_amount)
        byte_res = single_byte_template_attack(templates, attack_vectors, sample_function = attack_function, fun = fun, N = N)
