...
filename = @sprintf("traces/csc_aes_unmasked_%s.trs", bytes2hex(key))
touch(filename)
rm(filename)
...
function encrypt_log_trace(pt::MVector{16, UInt8})
    ...
    output = Logging.extractValue.(CSC.AES.AES_encrypt(ptl, kl))
...
trs = InspectorTrace(filename, 32, Float32, 2036)
