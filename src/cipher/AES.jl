module AES
    ...
    const c_sbox = convert(Vector{UInt8}, [
    ...
    const c_sboxi = convert(Vector{UInt8}, [
    ...
    const c_rcon = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36]
    ...
    sub_bytes(w::Vector) =  map(x -> c_sbox[x+1], w)
    inv_sub_bytes(w) = c_sboxi[w+1]
    inv_sub_bytes(w::Vector) = map(x -> c_sboxi[x+1], w)
    ...
    function AES_encrypt(pt::Vector{T}, k::Vector{T})::Vector{T} where T
        @assert length(pt) == 16
        key_schedule = key_expand(k)
        state = convert(Matrix{T}, (permutedims(reshape(pt, 4, 4), [2,1])))
    ...
    function AES_decrypt(ct::Vector{T}, k::Vector{T})::Vector{T} where T
        @assert length(ct) == 16
    ...
    function AES_encrypt_hex(pt, k)
        bytes2hex(AES_encrypt(hex2bytes(pt), hex2bytes(k)))
    end
    function AES_decrypt_hex(pt, k)
        bytes2hex(AES_decrypt(hex2bytes(pt), hex2bytes(k)))
    end
